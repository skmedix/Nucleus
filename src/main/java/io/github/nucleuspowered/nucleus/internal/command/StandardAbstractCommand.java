...
public abstract class StandardAbstractCommand<T extends CommandSource> implements CommandCallable {
    ...
    private void getSubcommandOf(Class<? extends StandardAbstractCommand> c, StringBuilder sb, boolean appendPeriod) {
     ...
     */
    protected abstract CommandResult executeCommand(SubjectPermissionCache<T> src, CommandContext args) throws Exception;
    ...
    // Warmups
    // -------------------------------------
    protected int getWarmup(final SubjectPermissionCache<Player> src) {
    ...
    @SuppressWarnings("unchecked")
    private ContinueMode setupWarmup(final SubjectPermissionCache<Player> src, CommandContext args) {
        if (bypassWarmup) {
            return ContinueMode.CONTINUE;
        }
        ...
        if (wc.isOnMove() && wc.isOnCommand()) {
            ...
            src.getSubject().sendMessage(NucleusPlugin.getNucleus().getMessageProvider().getTextMessageWithFormat("warmup.onMove"));
        } else if (wc.isOnCommand()) {
            src.getSubject().sendMessage(NucleusPlugin.getNucleus().getMessageProvider().getTextMessageWithFormat("warmup.onCommand"));
        }
    ...
    private void setCooldown(SubjectPermissionCache<Player> src, CommandContext args) {
        if (!args.hasAny(NoModifiersArgument.NO_COOLDOWN_ARGUMENT) && !permissions.testCooldownExempt(src)) {
            ...
            if (cooldownTime > 0) {
                ...
                cooldownStore.put(src.getSubject().getUniqueId(), Instant.now().plus(cooldownTime, ChronoUnit.SECONDS));
            }
        }
    }
     ...
     */
    private ContinueMode applyCost(SubjectPermissionCache<Player> src, CommandContext args) {
        double cost = getCost(src, args);
        if (cost == 0.) {
            return ContinueMode.CONTINUE;
        }

        if (!plugin.getEconHelper().withdrawFromPlayer(src.getSubject(), cost)) {
            return ContinueMode.STOP;
        }
    ...
    private class UsageCommand implements CommandCallable, CommandExecutor {
        ...
        public CommandResult process(CommandSource source, String arguments, @Nullable String previous) throws CommandException {
            ...
            StandardAbstractCommand<?> parent = StandardAbstractCommand.this;
